// CommunicationDLL.cpp : Defines the exported functions for the DLL application.
//
...
string abb_comm::pingRobot(int idCode)
{
	...
	sprintf_s(buff, "%.3d ", idCode); //identification code
	msg += buff;
	msg += "#";
...
string abb_comm::setCartesian(double x, double y, double z, double q0, double qx, double qy, double qz, int idCode)
{
	...
	sprintf_s(buff, "%.3d ", idCode); //identification code
	msg += buff;
	sprintf_s(buff, "%+08.1lf ", x);
	msg += buff;
	sprintf_s(buff, "%+08.1lf ", y);
	msg += buff;
	sprintf_s(buff, "%+08.1lf ", z);
	msg += buff;
	sprintf_s(buff, "%+08.5lf ", q0);
	msg += buff;
	sprintf_s(buff, "%+08.5lf ", qx);
	msg += buff;
	sprintf_s(buff, "%+08.5lf ", qy);
	msg += buff;
	sprintf_s(buff, "%+08.5lf ", qz);
	msg += buff;
	msg += "#";
...
string abb_comm::setJoints(double joint1, double joint2, double joint3, double joint4, double joint5, double joint6, int idCode)
{
	...
	sprintf_s(buff, "%.3d ", idCode); //identification code
	msg += buff;
	sprintf_s(buff, "%+08.2lf ", joint1);
	msg += buff;
	sprintf_s(buff, "%+08.2lf ", joint2);
	msg += buff;
	sprintf_s(buff, "%+08.2lf ", joint3);
	msg += buff;
	sprintf_s(buff, "%+08.2lf ", joint4);
	msg += buff;
	sprintf_s(buff, "%+08.2lf ", joint5);
	msg += buff;
	sprintf_s(buff, "%+08.2lf ", joint6);
	msg += buff;
	msg += "#";
...
string abb_comm::setSpeed(double tcp, double ori, int idCode)
{
	...
	sprintf_s(buff, "%.3d ", idCode); //identification code
	msg += buff;
	sprintf_s(buff, "%08.1lf ", tcp);
	msg += buff;
	sprintf_s(buff, "%08.2lf ", ori);
	msg += buff;
	msg += "#";
...
string abb_comm::setZone(bool fine, double tcp_mm, double ori_mm, double ori_deg, int idCode)
{
	...
	sprintf_s(buff, "%.3d ", idCode); //identification code
	msg += buff;
	sprintf_s(buff, "%.1d ", fine);
	msg += buff;
	sprintf_s(buff, "%.2lf ", tcp_mm);
	msg += buff;
	sprintf_s(buff, "%.2lf ", ori_mm);
	msg += buff;
	sprintf_s(buff, "%.2lf ", ori_deg);
	msg += buff;
	msg += "#";
...
string abb_comm::specialCommand(int command, double param1, double param2, double param3, double param4, double param5, int idCode)
{
	...
	sprintf_s(buff, "%.3d ", idCode); //identification code
	msg += buff;
	sprintf_s(buff, "%.1d ", command);
	msg += buff;
	sprintf_s(buff, "%+09.2lf ", param1);
	msg += buff;
	sprintf_s(buff, "%+09.2lf ", param2);
	msg += buff;
	sprintf_s(buff, "%+09.2lf ", param3);
	msg += buff;
	sprintf_s(buff, "%+09.2lf ", param4);
	msg += buff;
	sprintf_s(buff, "%+09.2lf ", param5);
	msg += buff;
	msg += "#";
...
string abb_comm::setDIO(int dio_number, int dio_state, int idCode)
{
	...
	sprintf_s(buff, "%.3d ", idCode); //identification code
	msg += buff;
	sprintf_s(buff, "%.2d ", dio_number);
	msg += buff;
	sprintf_s(buff, "%.2d ", dio_state);
	msg += buff;
	msg += "#";
	return (msg);
}
...
{
	int ok, idCode;
	sscanf_s(msg.c_str(), "%*d %d %d %*f %lf %lf %lf %lf %lf %lf %lf", &idCode, &ok, x, y, z, q0, qx, qy, qz);
	if (ok)
		return idCode;
	else
		return -1;
...
{
	int ok, idCode;
	sscanf_s(msg.c_str(), "%*d %d %d %*f %lf %lf %lf %lf %lf %lf", &idCode, &ok, joint1, joint2, joint3, joint4, joint5, joint6);
	if (ok)
		return idCode;
	else
		return -1;
